...
public class TypeDocumentDemandeReservationLogementRest implements Serializable {
	...
	@Inject private AssocierTypeDocumentDemandeReservationLogementCtlInterface  associerTypeDocumentDemandeReservationLogementCtl ;
	...
	@ApiResponse(
			...
			content = @Content(
			        ...
			        array = @ArraySchema ( schema = @Schema(implementation = TypeDocumentDemandeReservationLogement.class))
			                )
	        )
	@ApiResponse(
			responseCode = "500",
	...
	@Operation(
			summary = "Association des types de documents de demande réservation logement",
			description = "Association des types de documents de demande réservation logement"
			)
	public Response creerModeFinancement(
						@RequestBody(
							description = "Associer des caracteristiques de demande de réservation logement", 
							required = true,
							content = @Content(schema = @Schema(implementation = TypeDocumentDemandeReservationLogement.class)))
		...
		try {
			...
			rtnList =  associerTypeDocumentDemandeReservationLogementCtl
					        .AssocierTypeDocumentDemandeReservationLogementList(entityList, mustUpdateExistingNew, namedGraph, isFetchGraph, locale, loggedInUser, msgList);
...
/************************************ Modification Association des types de documents de demande réservation logement *******************************************************/
	...
	@Operation(
			summary = "Modification d'association des types de documents de demande réservation logement",
			description = "Modification d'association des types de documents  de demande réservation logement"
			)
	public Response modifierModeFinancement(
						@RequestBody(
							description = "Modifier une association des caracteristiques de demande réservation logement", 
							required = true,
							content = @Content(schema = @Schema(implementation = TypeDocumentDemandeReservationLogement.class)))
						@Context HttpServletRequest servletRequest,
						List<TypeDocumentDemandeReservationLogement>  entityList) {
