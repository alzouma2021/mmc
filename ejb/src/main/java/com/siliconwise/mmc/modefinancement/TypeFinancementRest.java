...
@Stateless
@Path("/typeFinancements")
@OpenAPIDefinition(info = @Info(title = "Informations sur les types de financement: ", version = "1.0", description = "Informations sur les types de financements", contact = @Contact(url = "https://bitbucket.org/siliconwise/", name = "Silicon Wise"), license = @License(name = "License", url = "https://bitbucket.org/siliconwise/")))
public class TypeFinancementRest implements Serializable {

	
			private static final long serialVersionUID = 1L;
		
			@Inject private  ReferenceCtlInterface referenceCtl ;
		//	@Inject private SessionDAO sessionDAO;
			...
			@ApiResponse(
					responseCode = "204",
					description = "Aucun type financement trouvé"
			        )
			@ApiResponse(
					responseCode = "200",
					description = "Types financements  trouvés",
					content = @Content(
					        mediaType = MediaType.APPLICATION_JSON,
					        array = @ArraySchema ( schema = @Schema(implementation = TypeFinancement.class) )
					                )
			        )
			@ApiResponse(
					responseCode = "500",
			...
			@Operation(
					summary = "Trouver les types de financement",
					description = "Rechercher des types de financement"
					)
			public Response tousLesOperateursCritere(
				...
				List<TypeFinancement> rtnList = null;

				try {
				
					rtnList =  referenceCtl.tousLesTypeFinancements();
