...
@Stateless
@Path("/pays")
@OpenAPIDefinition(info = @Info(title = "Informations sur les pays: ", version = "1.0", description = "Informations sur les pays", contact = @Contact(url = "https://bitbucket.org/siliconwise/", name = "Silicon Wise"), license = @License(name = "License", url = "https://bitbucket.org/siliconwise/")))
public class PaysRest implements Serializable {
	...
	private static transient Logger logger = LoggerFactory.getLogger(PaysRest.class) ;
	...
	@ApiResponse(
			...
			content = @Content(
			        ...
			        array = @ArraySchema ( schema = @Schema(implementation = Pays.class))
			                )
	        )
	@ApiResponse(
			responseCode = "500",
